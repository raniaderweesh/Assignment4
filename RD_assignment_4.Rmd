...
title: | 
  ...
  | 2022-2023
author: |
  | Rania-Maria Derweesh
  | r.m.derweesh@students.uu.nl
  | http://www.github.com/raniaderweesh
date: April 2023
urlcolor: purple
linkcolor: purple
output: 
...
The given data suggests that the mean is different between the two given variables. "growth" variable has a mean of 2.46 and "rgdp60" has a mean of 5283.32. In addition, the standard deviation also differs, from 1.28 to 2439.39.
...
The purpose of including the variable "rgdp60" in the linear model is to adjust for the effect of the initial level of real GDP per capita in 1960. The "rgdp60" variable represents the real GDP per capita in 1960, and including it as a control variable allows us to analyse the relationship between the "treat" variable and economic growth while, also accounting for the initial economic conditions of the presented countries.
...
list(model1, model2, model3, model4) |>
  modelsummary(stars=T,gof_map = c("nobs", "r.sqaured"))
...
In order to determine the main driver of economic growth, we ought to analyse the R-squared values in the model summaries. The R-squared value indicates the proportion of the variance in the dependent variable (growth) that can be explained by the independent variables in the scheme 

By comparing the R-squared values of the models, the highest R-squared suggests that independent variables are the main drivers of economic growth. Therefore, we need to check the R-squared values in the model summaries to determine the main driver of economic growth.
